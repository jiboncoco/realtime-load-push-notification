...
POST   /outlets                    buat outlet (+ minimal 1 platform)
...
POST   /outlets/:id/tickets        ambil antrian (auto-assign platform) → {label, id}
...
**Trigger:**
- **Sisa 3 (CUS-5)**: tiap antrian platform berkurang, hitung posisi; jika `posisi == 3` & `reminded_3=false` → push, set `reminded_3=true` (idempoten).
