...
- **Hardening & QR (T-006)**: rate limiting via `lib/rateLimit.ts` (fixed-window in-memory per IP, X-Forwarded-For) dipasang di booking (10/mnt) & push-subscribe (20/mnt) → `Errors.tooManyRequests` (429). CI di `.github/workflows/ci.yml` (backend tsc+`bun test`; matrix apps tsc+build). QR outlet di CMS `app/outlets/[id]/QrCard.tsx` (lib `qrcode`, unduh PNG + cetak) → `NEXT_PUBLIC_CUSTOMER_BASE/o/{id}`. App TV responsif (font/padding skala HP→TV). **Google OAuth = Phase 2** (masih 501).
